...
import {
  ...
  updatePortfolioStartingBalance, getDataSourceStatus,
...
import type { Asset, Portfolio, Holding, Signal, Trade, AlertSettings, AlertHistoryItem, PriceSnapshot, DataSourceStatus, TrendSignal, TrendSourceStatus } from '@/lib/types';
...
function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      const [sigData, tradeData, settingsData, historyData, sourceData, trendData, trendSourceData] = await Promise.all([
        getRecentSignals(20),
        getRecentTrades(30),
        getAlertSettings(),
        getAlertHistory(20),
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Real-time market intelligence &middot; <span className="text-amber-400">Paper Trading</span></p>
              </div>
            </div>
...
function DashboardView({
  portfolio, assets, prices, signals, holdings, dataSources, onAnalyze, onAutoTrade, onRefresh, actionLoading
}: {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-900/20 border border-amber-800/30">
        ...
        <p className="text-xs text-amber-300/80">
          Paper Trading mode: all prices are real (live from Yahoo Finance & CoinGecko), but trades use virtual money. No real funds are at risk.
        </p>
      </div>
