...
const MODEL_VERSION = "cognitive_v1";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const asset of assets as AssetRow[]) {
      ...
      const dataSource = asset.asset_type === "crypto" ? "coingecko" : "yahoo_finance";
      ...
      const { action, confidence, reasoning, riskLevel, targetPrice, stopLoss } = generateSignal(indicators, currentPrice, trendSummary);

      let aiAnalysis: string | null = null;
      if (action !== "HOLD" || confidence > 60) {
        aiAnalysis = await getGeminiAnalysis(asset.symbol, asset.name, indicators, currentPrice, action, reasoning, trendSummary);
      }
    ...
    return new Response(
      JSON.stringify({
        message: `Generated ${insertedCount} cognitive signals (model: ${MODEL_VERSION}, ${trendCount} trend signals incorporated)`,
