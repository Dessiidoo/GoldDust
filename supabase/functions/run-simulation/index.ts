...
Deno.serve(async (req: Request) => {
  ...
  try {
      ...
      if (signal.action === "BUY" && signal.confidence >= minConfidence) {
        ...
        if (!dryRun) {
          tradesToInsert.push({
            ...
            signal_id: signal.id,
            status: "EXECUTED",
            notes: `Auto-buy: ${signal.reasoning.substring(0, 200)}`,
          });
        }
        ...
        if (!dryRun) {
          tradesToInsert.push({
            ...
            signal_id: signal.id,
            status: "EXECUTED",
            notes: `Auto-sell: ${signal.reasoning.substring(0, 200)}`,
          });
        }
    ...
    return new Response(
      JSON.stringify({
        message: dryRun
          ? `Dry run: would execute ${details.length} trades`
          : `Executed ${details.length} trades`,
        dry_run: dryRun,
