...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: recentSignals } = await supabase
      .from("signals")
      .select(`
        id, action, confidence, reasoning, ai_analysis, target_price, stop_loss, risk_level, created_at,
        assets:asset_id(symbol, name),
        price_snapshots!inner(asset_id, price)
      `)
    ...
    const formattedSignals = eligibleSignals.map((s) => ({
      symbol: s.assets?.symbol ?? "Unknown",
      name: s.assets?.name ?? "Unknown",
      action: s.action,
      confidence: s.confidence,
      reasoning: s.reasoning,
      ai_analysis: s.ai_analysis,
      current_price: s.price_snapshots?.[0]?.price ?? 0,
    ...
    const alertLogs = eligibleSignals.map((s) => ({
      ...
      message: `${s.assets?.symbol}: ${s.action} at ${s.confidence}% confidence`,
    ...
    return new Response(
      JSON.stringify({
        message: sent ? `Sent alert email to ${settings.email}` : "Email send failed",
        sent: sent ? 1 : 0,
